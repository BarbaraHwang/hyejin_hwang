...
/*   $('.full .coin_d').on({
    'dragstart': function () {
      $('.full .coin_d.on img.changeImg').draggable();
  ...
  /*   function newPage() {
      let ranPages = $('.full .card2 .web .web_box').get();
      let ranPage_index = Math.floor(Math.random() * ranPages.length);
      let page_random = ranPages[ranPage_index];
      $(page_random);
      console.log();
    } */
  ...
  window.addEventListener('load',function () {
    ...
    $('.full .card2 .web .web_box').mouseover(function () {
      let h = Math.ceil($(this).find('img').height()-460);
      $(this).find('img').animate({'top':-h},500);
    })
